...
@section('content_header')
    <h1 class="text-primary font-weight-bold">
        Medicine List - Patient Code: {{ $patientcode }}
...
<div class="card card-outline card-primary shadow">
    ...
    <div class="card-body" id="medicineTableArea">
        @include('admission.admitmedicine.medicine_rows')
    </div>
</div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <input type="hidden" name="order_type" id="order_type" value="admit">
                        <div class="col-md-6 form-group">
...
function getSlipHtml() {
    ...
    return `
        ...
        <html>
        <head>
            <title>Admit Medicine Prescription</title>
            <style>
...
function printSlip() {
    ...
    printWindow.document.title = 'Admit_Medicine_' + patientname + '_' + today;
...
function saveSlip() {
    $.ajax({
        url: "{{ route('admission.admedicineSaveAllAdmit') }}",
        type: "POST",
...
function loadMedicineTable() {
    $.ajax({
        url: "{{ route('admission.admedicineList.list') }}",
        type: "POST",
        data: { _token: "{{ csrf_token() }}", patientcode: $('#patientcode').val() },
        success: function (response) {
...
$(document).ready(function () {
    ...
    $('#medicineForm').on('submit', function (e) {
        e.preventDefault();
        $.ajax({
            url: "{{ route('admission.admedicineSave.save') }}",
            type: "POST",
            data: $(this).serialize(),
            success: function (response) {
    ...
    $(document).on('click', '.deleteMedicineBtn', function () {
        ...
        var id = $(this).data('id');
        $.ajax({
            url: "{{ route('admission.admedicine.delete') }}",
            type: "POST",
...
@push('js')
@if(session('active_page') == 'admedicine')
<script>
$(document).ready(function () {
    let menu = $('a[href*="admission/admedicine"]');
