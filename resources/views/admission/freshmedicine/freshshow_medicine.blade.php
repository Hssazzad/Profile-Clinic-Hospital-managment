...
@section('content_header')
    <h1 class="text-primary font-weight-bold">
        Fresh Prescription- Patient Code: {{ $patientcode }}
    ...
    <div class="card-header">
        ...
        <div class="card-tools">
            <button type="button" onclick="savePrintNext()" class="btn btn-success btn-sm">
                <i class="fas fa-save"></i> Print & Next
            </button>
        </div>
    </div>
    <div class="card-body" id="medicineTableArea">
        @include('admission.freshmedicine.freshmedicine_rows')
    </div>
</div>
...
function printSlip() {
    ...
    printWindow.document.write(`
        ...
        <html>
        <head>
            <title>Fresh Prescription</title>
            <style>
    ...
    printWindow.document.title = 'Fresh_Prescription_' + patientname + '_' + today + '_' + time;
...
function savePrintNext() {
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.success) {
                $('#ajaxMessage').html('<div class="alert alert-success">Printed. ' + res.message + '</div>');
