                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-md-6 form-group">
                            ...
                            <input type="text" name="order_type" id="order_type" class="form-control" value="round">
                        </div>
                        <div class="col-md-6 form-group">
...
function printSlip() {
    ...
    printWindow.document.write(`
        ...
        <html>
        <head>
            <title>Round Patient Medicine Prescription</title>
            <style>
        ...
        <body>
            ...
            <div style="text-align:center; font-size:22px; font-weight:bold; margin:15px 0; text-decoration:underline;">
                ROUND PATIENT MEDICINE PRESCRIPTION
            </div>
    ...
    printWindow.document.title = 'Round_Medicine_' + patientname + '_' + today + '_' + time;
...
function savePrintNext() {
    $.ajax({
        url: "{{ route('admission.roundmedicineSaveAll.save') }}",
        type: "POST",
...
$(document).ready(function () {
    ...
    $('#saveAllBtn').on('click', function () {
        $.ajax({
            ...
            error: function () {
                $('#ajaxMessage').html(
                    '<div class="alert alert-danger">Save All failed.</div>'
                );
            }
        });
    });
    ...
    $('#medicineForm').on('submit', function (e) {
        e.preventDefault();
        $.ajax({
            url: "{{ route('admission.roundmedicineSave.save') }}",
            type: "POST",
            data: $(this).serialize(),
            success: function (response) {
    ...
    $(document).on('click', '.deleteMedicineBtn', function () {
        ...
        var id = $(this).data('id');
        $.ajax({
            url: "{{ route('admission.roundmedicine.delete') }}",
            type: "POST",
    ...
    function loadMedicineTable() {
        $.ajax({
            url: "{{ route('admission.roundmedicineList.list') }}",
            type: "POST",
            data: { _token: "{{ csrf_token() }}", patientcode: $('#patientcode').val() },
            success: function (response) {
