...
@section('title', 'Pre-Surgery | Professor Clinic')

@section('content_header')
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h1 class="m-0 page-main-title">
                <span class="page-title-icon"><i class="fas fa-syringe"></i></span>
...
@php
    ...
    $PreSurgeryPatients = $PreSurgeryPatients ?? collect();
@endphp
...
<div class="step-wrapper mb-3">
    ...
    <div class="step-item-wrap">
        ...
        <div class="step-info">
            ...
            <div class="step-sub" id="step2-sublabel">Pre-Op medicines &amp; details</div>
        </div>
    </div>
</div>
...
<div class="gov-panel" id="past-rx-card">
    ...
    <div class="gov-panel-header">
        ...
        <div class="gov-panel-meta">
            <span class="gov-badge gov-badge-red">
                ...
                {{ $PreSurgeryPatients->total() ?? $PreSurgeryPatients->count() }} records
            </span>
        </div>
    </div>
    ...
    <div class="gov-table-wrap">
        <table class="gov-table" id="nursingRxTable">
            ...
            <tbody id="nursingRxTableBody">
                @forelse($PreSurgeryPatients as $np)
                @php
                    $npRxId   = $np->id                ?? '';
                ...
                <tr>
                    <td colspan="9">
                        <div class="empty-state">
                            ...
                            <p>No past pre-surgery prescriptions found.</p>
                        </div>
                    </td>
                </tr>
                @endforelse
    ...
    @if(method_exists($PreSurgeryPatients,'links'))
    <div class="gov-pagination-bar">
        <div class="pagination-info">
            <i class="fas fa-list mr-1"></i>
            Showing <strong>{{ $PreSurgeryPatients->firstItem() ?? 0 }}</strong>
            to <strong>{{ $PreSurgeryPatients->lastItem() ?? 0 }}</strong>
            of <strong>{{ $PreSurgeryPatients->total() ?? 0 }}</strong> records
        </div>
        {{ $PreSurgeryPatients->links('pagination::bootstrap-4') }}
    </div>
    @endif
</div>
...
<div id="panel-step2" style="display:none;">
    ...
    <div class="patient-selected-bar patient-selected-bar-danger mb-4">
        ...
        <div class="psb-right">
            ...
            <span class="psb-status-label">Pre-Surgery Patient</span>
    ...
    <div id="admission-loading" class="admission-status-bar admission-loading-bar" style="display:none;">
        <i class="fas fa-spinner fa-spin mr-2"></i>Pre-Order medicines লোড হচ্ছে...
    </div>
    <div id="no-admission-alert" class="admission-status-bar admission-warning-bar" style="display:none;">
        <i class="fas fa-exclamation-triangle mr-2"></i>
        কোনো <strong>On Admission</strong> রেকর্ড পাওয়া যায়নি। তবুও Pre-Surgery Prescription দেওয়া যাবে।
    </div>
    ...
    <div class="modern-card" id="rx-form-section">
        <div class="modern-card-header">
            <div class="modern-card-title">
                ...
                <div>
                    <h5 class="mb-0 font-weight-bold">Pre-Operative Prescription</h5>
                    <small class="text-muted">Fill in the pre-surgery clinical information</small>
                </div>
            </div>
        </div>
        <div class="modern-card-body">
            ...
            <div class="section-heading mb-3">
                ...
                <span>Patient &amp; Pre-Op Information</span>
            </div>
                ...
                <div class="col-md-6">
                    ...
                    <div class="modern-field-group">
                        <label class="modern-label">
                            <i class="fas fa-clock mr-1 text-danger" style="font-size:11px;"></i>OT Time
                        </label>
                        <div class="input-with-icon">
                            <i class="fas fa-syringe input-icon text-danger"></i>
                            <input type="time" class="modern-input with-icon" id="f-ot-time">
                        </div>
                    </div>
                </div>
            </div>
                            ...
                            <tr class="empty-row" id="empty-med-row">
                                <td colspan="9">
                                    <div class="med-empty-state">
                                        ...
                                        <span>Patient select করলে Pre-Order medicines auto-load হবে।</span>
                                    </div>
                                </td>
                            </tr>
                        </tbody>
        ...
        <div class="row mb-4">
            ...
            <div class="col-lg-3 col-sm-6 mb-3 mb-lg-0">
                <div class="rx-summary-card rx-card-orange">
                    ...
                    <div class="rx-summary-content">
                        <div class="rx-summary-label">OT Time</div>
                        <div class="rx-summary-value" id="ib-ottime">—</div>
                    </div>
                </div>
            </div>
            ...
            <div class="modern-card-body p-0">
                <div id="prescription-print-area">
                    <div class="rx-wrapper">
                        ...
                        <div class="rx-section-center">Pre-Operative Order On</div>
                        <div class="rx-time-right" id="rx-ottime-display"></div>
    ...
    <div class="modal-dialog modal-lg modal-dialog-centered modal-dialog-scrollable" role="document">
        <div class="modal-content rx-modal-content">
            <div class="modal-header rx-modal-header">
                <div class="d-flex align-items-center">
                    <div class="rx-modal-icon mr-3"><i class="fas fa-syringe"></i></div>
                    <div>
                        <h5 class="modal-title mb-0 font-weight-bold text-white">Pre-Surgery Prescription</h5>
                ...
                <div id="modal-rx-area" class="d-none">
                    ...
                    <div id="modal-prescription-print-area" style="padding:20px 24px;">
                        <div class="rx-wrapper">
                            ...
                            <div style="margin:10px 0 3px;font-size:13px;"><span class="rx-symbol">Rx</span></div>
                            <div class="rx-section-center">Pre-Operative Order On</div>
                            <div class="rx-time-right" id="m-rx-ottime-display"></div>
...
.step-title-active { color:#C62828; } .step-title-done { color:#2E7D32; } .step-title-idle { color:#aaa; }
...
.fbar-search { flex:1;min-width:240px; } .fbar-count { flex-shrink:0; }
...
.fsg-btn-red { background:var(--red-mid);color:#fff; } .fsg-btn-red:hover { background:var(--red-deep); }
...
.btn-psb-change { background:rgba(255,255,255,.18);border:1.5px solid rgba(255,255,255,.4);color:#fff;border-radius:var(--radius-sm);padding:5px 13px;font-size:12px;font-weight:600;cursor:pointer;transition:all .2s; }
.btn-psb-change:hover { background:rgba(255,255,255,.3); }
.admission-status-bar { border-radius:var(--radius-sm);padding:10px 16px;font-size:13px;font-weight:500;margin-bottom:14px;display:flex;align-items:center; }
.admission-loading-bar { background:#e3f2fd;color:#1565C0;border:1px solid #bbdefb; }
.admission-warning-bar { background:#fff8e1;color:#e65100;border:1px solid #ffe082; }
.section-heading { display:flex;align-items:center;font-size:13.5px;font-weight:700;color:var(--text-primary); }
.section-divider { border:none;border-top:1.5px solid var(--border);margin:20px 0; }
.section-divider-full { display:flex;align-items:center;justify-content:space-between;border-top:1.5px solid var(--border);padding-top:16px; }
.modern-label { font-size:12px;font-weight:700;color:var(--text-muted);text-transform:uppercase;letter-spacing:.4px;margin-bottom:5px;display:block; }
.modern-input { width:100%;border:1.5px solid var(--border);border-radius:var(--radius-sm);padding:8px 12px;font-size:13.5px;color:var(--text-primary);transition:border-color .2s;background:#fff; }
.modern-input:focus { border-color:var(--red-mid);outline:none;box-shadow:0 0 0 2px rgba(198,40,40,.1); }
.modern-field-group { margin-bottom:14px; }
.input-with-icon { position:relative; }
.input-icon { position:absolute;left:11px;top:50%;transform:translateY(-50%);font-size:13px;pointer-events:none; }
.modern-input.with-icon { padding-left:32px; }
.med-table-card { border:1.5px solid var(--border);border-radius:var(--radius-md);overflow:hidden; }
.selected-med-card-danger { border-color:#ffcdd2; }
.med-table-card-header { padding:10px 14px;background:#fafbfd;border-bottom:1px solid var(--border);display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:8px; }
.med-table-dot { width:8px;height:8px;border-radius:50%;margin-right:8px;flex-shrink:0; }
.med-table-title { font-size:12.5px;font-weight:700;color:var(--text-primary); }
.med-count-pill { background:#e8eaf6;color:#283593;border-radius:10px;padding:2px 9px;font-size:11px;font-weight:700;margin-left:8px; }
.med-count-pill-danger { background:var(--red-light);color:var(--red-mid); }
.avail-filter-wrap { display:flex;align-items:center;background:#fff;border:1.5px solid var(--border);border-radius:var(--radius-sm);overflow:hidden;max-width:220px; }
.avail-filter-icon { padding:0 9px;color:#aaa;font-size:12px;flex-shrink:0; }
.avail-filter-input { flex:1;border:none;outline:none;padding:6px 4px;font-size:12.5px;color:var(--text-primary); }
.btn-med-action { border-radius:var(--radius-sm);padding:6px 14px;font-size:12px;font-weight:600;border:1.5px solid transparent;cursor:pointer;transition:all .2s;display:inline-flex;align-items:center; }
.btn-med-add-danger { background:var(--red-light);color:var(--red-mid);border-color:var(--red-soft); }
.btn-med-add-danger:hover { background:var(--red-mid);color:#fff; }
.btn-med-clear-danger { background:transparent;border:1.5px solid var(--red-soft);color:var(--red-mid);border-radius:var(--radius-sm);padding:4px 12px;font-size:12px;font-weight:600;cursor:pointer;transition:all .2s;display:inline-flex;align-items:center; }
.btn-med-clear-danger:hover { background:var(--red-mid);color:#fff; }
.med-table { font-size:12.5px; }
.med-table thead th { background:#f5f6f8;color:var(--text-muted);font-size:11px;font-weight:700;text-transform:uppercase;letter-spacing:.5px;padding:7px 10px;border-bottom:1.5px solid var(--border); }
.med-table tbody td { padding:6px 8px;vertical-align:middle;border-bottom:1px solid #f0f0f0; }
.med-table .form-control-sm { height:30px;font-size:12px;padding:3px 7px; }
.med-empty-state { display:flex;align-items:center;justify-content:center;gap:10px;padding:18px;color:#b0bec5;font-size:13px; }
.med-empty-state i { font-size:20px; }
.avail-med-name { font-size:12.5px;font-weight:600;color:var(--text-primary); }
.avail-med-row:hover { background:#fff8f8; }
.modern-checkbox { width:16px;height:16px;accent-color:var(--red-mid);cursor:pointer; }
.btn-quick-add { width:26px;height:26px;border-radius:50%;border:1.5px solid;cursor:pointer;font-size:11px;display:inline-flex;align-items:center;justify-content:center;transition:all .2s; }
.btn-quick-add-green { background:var(--green-light);color:var(--green-mid);border-color:#a5d6a7; }
...
.btn-rx-print { background:var(--blue-deep);color:#fff;border-color:var(--blue-deep); } .btn-rx-print:hover { background:var(--blue-mid); }
.btn-rx-edit  { background:#fff7e0;color:#bf360c;border-color:#ffcc80; } .btn-rx-edit:hover { background:#bf360c;color:#fff; }
.btn-rx-new   { background:#f0f4f8;color:var(--text-primary);border-color:var(--border); } .btn-rx-new:hover { background:#e8ecf2; }
...
.msi-red    { background:linear-gradient(135deg,#ffebee,#fff); } .msi-green  { background:linear-gradient(135deg,#e8f5e9,#fff); }
.msi-orange { background:linear-gradient(135deg,#fff3e0,#fff); } .msi-teal   { background:linear-gradient(135deg,#e0f2f1,#fff); }
.modal-summary-item>i { font-size:16px;flex-shrink:0; }
.msi-red>i { color:var(--red-mid); } .msi-green>i { color:var(--green-deep); } .msi-orange>i { color:var(--orange); } .msi-teal>i { color:var(--teal-mid); }
...
/* ══ AUTO-LOAD PRE-ORDER MEDICINES ══ */
function fetchAndLoadPreOrderMeds(){
    showEl('admission-loading'); hideEl('no-admission-alert');
    document.getElementById('adm-loaded-note').classList.add('d-none');
    fetch('/nursing/presurgery/preorder-medicines',{method:'GET',headers:{'Accept':'application/json','X-CSRF-TOKEN':CSRF_TOKEN}})
    .then(function(res){ hideEl('admission-loading'); if(!res.ok){showAlert('warning','<i class="fas fa-exclamation-circle mr-1"></i> Pre-Order medicines লোড হয়নি।'); return null;} return res.json(); })
    .then(function(data){
        if(!data) return;
        if(!data.success||!data.rows||!data.rows.length){hideEl('admission-loading'); return;}
        selectedMeds=[];
    ...
    .catch(function(err){ hideEl('admission-loading'); console.warn('Pre-Order fetch error:',err); });
}
...
function selectPatient(btn){
    ...
    document.getElementById('f-ot-time').value      ='';
...
function saveAndGenerateRx(){
    ...
    var payload={patient_id:patientId,patient_name:gVal('f-patient-name'),patient_age:gVal('f-patient-age'),prescription_date:gVal('f-date'),ot_time:gVal('f-ot-time'),notes:'',medicines:medsToSave};
    function trySave(idx){
        if(idx>=PRE_STORE_URLS.length){btn.disabled=false;btn.innerHTML='<i class="fas fa-save mr-1"></i> Save &amp; Generate Prescription';showAlert('danger','Could not save. Route not found.');return;}
        fetch(PRE_STORE_URLS[idx],{method:'POST',headers:{'X-CSRF-TOKEN':CSRF_TOKEN,'Accept':'application/json','Content-Type':'application/json'},body:JSON.stringify(payload)})
        ...
        .then(function(data){
            ...
            generateRxView(data.prescription_id||'');
            showAlert('success','<i class="fas fa-check-circle mr-1"></i> Prescription saved! ID: <strong>#'+(data.prescription_id)+'</strong>');
        })
...
function generateRxView(savedId){
    ...
    setText('ib-ottime', fmtTime(gVal('f-ot-time')));
    ...
    var otEl=document.getElementById('rx-ottime-display');
    if(otEl){ var t=fmtTime(gVal('f-ot-time')); otEl.textContent=(t!=='—')?t:''; }
...
function viewPrescription(prescriptionId){
    ...
    $.ajax({url:'/nursing/presurgery/detail/'+prescriptionId,method:'GET',dataType:'json'})
...
function populateModal(d){
    ...
    var otEl=document.getElementById('m-rx-ottime-display'); if(otEl){otEl.textContent=d.ot_time?fmtTime(d.ot_time):'';}
    ...
    meds.filter(function(m){return m&&(m.medicine_name||'').trim();}).forEach(function(m){
        var parts=[m.medicine_name];
        if(m.dose) parts.push(m.dose); if(m.route) parts.push(m.route); if(m.frequency) parts.push(m.frequency);
