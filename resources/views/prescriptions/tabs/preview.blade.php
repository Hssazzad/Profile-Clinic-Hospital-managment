...
@endphp
@php
  // Build doctor display text from prescription_doctors list
  $docLine = '—';

  if (!empty($rxDoctors) && ($rxDoctors instanceof \Illuminate\Support\Collection ? $rxDoctors->count() : count($rxDoctors))) {
      $docLine = collect($rxDoctors)->map(function($d){
          $name = $d->doctor_name ?? $d->name ?? '';
          $role = $d->role ?? '';
          return trim($name . ($role ? " ({$role})" : ''));
      })->filter()->values()->join(', ');
  } elseif (!empty($rx?->doctor_name)) {
      // fallback (old single-doctor system)
      $docLine = $rx->doctor_name;
  }
@endphp
  ...
  <div id="printableArea">
    ...
    <div class="print-header">
      ...
      <div class="clinic-right">
        <div><b>Prescription No:</b> {{ $pid }}</div>
        <div><b>Date:</b> {{ ($rx->prescribed_on ?? now()->toDateString()) }}</div>
        <div><b>Patient ID:</b> {{ $patient->patientcode ?? '—' }}</div>
      </div>
    </div>

    {{-- Top strip --}}
    <div class="rx-top-strip">
      <div><b>Doctor:</b> {{ $docLine }}</div>

		{{-- Optional: if you also want to show reg/speciality, only possible if you load them --}}
		@if(!empty($rxDoctors) && collect($rxDoctors)->first()?->speciality)
		  <div><b>Speciality:</b>
			{{ collect($rxDoctors)->pluck('speciality')->filter()->unique()->values()->join(', ') }}
		  </div>
		@endif
    ...
    <div class="box">
        ...
        <table class="rx">
         <thead>
          <tr>
            ...
            <th>Strength (শক্তি)</th>
            <th>Dose (মাত্রা)</th>
            <th>Frequency (ফ্রিকোয়েন্সি)</th>
            <th>Duration (সময়কাল)</th>
            <th>Instructions (খাওয়ার নিয়ম)</th>
          </tr>
        </thead>
      ...
      @else
        <em>No Medicine Added</em>
      @endif
    </div>
    {{-- Investigations --}}
    <div class="box">
      <h3>Exammination/Test</h3>
